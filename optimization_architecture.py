# -*- coding: utf-8 -*-
"""OptimizationArchitekturenVGG16 .ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1YPQWN7-jWQMg5fxPaKpIeCzG1XND9tdl
"""
...
from tensorflow.keras.layers import Dense, GlobalAveragePooling2D, Dropout  # Dropout importieren VGG16
...
# Bildpfade korrigieren (falls erforderlich)
...
# Trainingsdaten-Augmentation hinzufügen
train_datagen_augmented = ImageDataGenerator(
    rescale=1./255,
    rotation_range=45,        # Zufällige Drehungen von Bildern im Bereich von 0 bis 45 Grad
    width_shift_range=0.2,    # Zufällige horizontale Verschiebung
    height_shift_range=0.2,   # Zufällige vertikale Verschiebung
    shear_range=0.2,          # Scherungsintensität
    horizontal_flip=True,     # Horizontales Spiegeln
    fill_mode='nearest'       # Füllmodus für neu erstellte Pixel
)

# Ersetze die train_datagen-Instanz durch die neue train_datagen_augmented-Instanz
train_generator = train_datagen_augmented.flow_from_dataframe(
    ...
    color_mode='grayscale',   # Hier wird color_mode als 'grayscale' gesetzt
    class_mode='raw',
    target_size=(224, 224),
    batch_size=18
)

# Validierungsgenerator mit bereinigten Validierungsdaten erstellen
...
# Funktion zur Erweiterung des Eingabebildes von 1 auf 3 Kanäle
def expand_dims(input):
    return tf.tile(input, [1, 1, 1, 3])

# Beispiel Tensor mit einer Kanal
input_tensor = tf.zeros([1, 224, 224, 1])

# Lambda Layer anwenden
expanded_tensor = Lambda(expand_dims)(input_tensor)
...
# Eingabeschicht für einfarbige Bilder erstellen
input_tensor = Input(shape=(224, 224, 1))

# Einfarbiges Bild auf drei Kanäle erweitern
x = Lambda(lambda x: tf.tile(x, [1, 1, 1, 3]))(input_tensor)

# Basis Xception Modell laden, ohne die obersten Schichten und mit angepasster Eingabeschicht
base_model = VGG16(weights='imagenet', include_top=False, input_tensor=x)

# Ausgabe des Basis-Modells
x = base_model.output
reg = l1_l2(l1=0.00001, l2=0.00001)
# Weiterführende Schichten
# Weiterführende Schichten
x = GlobalAveragePooling2D()(x)
# Dritte Dense-Schicht
x = BatchNormalization()(x)
x = Dense(2048, activation='relu', kernel_regularizer=reg)(x)
# Dritte Dense-Schicht
x = BatchNormalization()(x)
...
# Konfigurieren des ReduceLROnPlateau Callbacks
reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=5, min_lr=0.001)

# Log-Verzeichnis für TensorBoard erstellen
log_dir = os.path.join("logs", "fitvgg1", datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
tensorboard_callback = TensorBoard(log_dir=log_dir, histogram_freq=1)


# %load_ext tensorboard
# %tensorboard --logdir logs/fitvgg1


# Optimierung durchführen
best = fmin(fn=objective, space=search_space, algo=tpe.suggest, max_evals=50, trials=trials)
print("Beste gefundene Hyperparameter:", best)

# Beste Parameter extrahieren und umwandeln
best_params = {
    'learning_rate': best['lr'],
    'epochs': [10, 20, 30][best['epochs']]  # Zugriff auf die tatsächliche Epochenanzahl
}

print("Verwendete Hyperparameter für das finale Training:", best_params)

# Modell mit den besten Parametern neu konfigurieren und trainieren
model.compile(optimizer=Adam(learning_rate=best_params['learning_rate']),
              loss='mean_squared_error',
              metrics=['mean_squared_error', 'mean_absolute_error'])

# Training starten
history = model.fit(train_generator, epochs=best_params['epochs'], validation_data=val_generator, callbacks=[tensorboard_callback, reduce_lr, EarlyStopping(patience=10, min_delta=0.001, restore_best_weights=True)])
...
# Speichern der Trainingsmetriken als CSV in Google Drive
results_df.to_csv('/content/drive/My Drive/CNNMODEL/Training_Metricsmit1Inputvgg1.csv', index=False)



# Berechne die Anzahl der Schritte pro Epoche für den Testgenerator
test_steps_per_epoch = np.ceil(test_generator.samples / test_generator.batch_size)

# Bewerte das Modell auf den Testdaten
test_loss, test_mse, test_mae = model.evaluate(test_generator, steps=test_steps_per_epoch)

# Jetzt kannst du deine Testergebnisse sicher verwenden
test_results = {
    "Metrik": ["Loss", "MSE", "MAE"],
...
# Speichere die Testmetriken als CSV in Google Drive
test_results_df.to_csv('/content/drive/My Drive/CNNMODEL/Test_Metrics_mit_1_Inputvgg1.csv', index=False)



# Speichern des Modells in Google Drive
model_save_path = '/content/drive/My Drive/CNNMODEL/Mein_Trainiertes_Modellmi1Input-vgg1'
...
# Visualisierung des Trainings- und Validierungs-MSE
...
# Visualisierung des Trainings- und Validierungs-MAE
plt.figure(figsize=(12, 6))
...
# Sammle alle Hyperparameter in einem Dictionary
hyperparameters = {
    'learning_rate': 0.00281,
    'batch_size': 18,
    'epochs': 300,
    'dropout_rates': [0.25, 0.25],  # Beispielsweise für zwei Dropout-Schichten
    'regularization_l1': 0.00001,
    'regularization_l2': 0.00001,
    'rotation_range': 45,
...
hyperparameters_df = pd.DataFrame(list(hyperparameters.items()), columns=['Hyperparameter', 'Wert'])

# Speichere den DataFrame als CSV
csv_save_path = '/content/drive/My Drive/CNNMODEL/Hyperparametervgg1.csv'
...
# Hyperparameter in einen DataFrame umwandeln und speichern
hyperparameters_df = pd.DataFrame(list(best_params.items()), columns=['Hyperparameter', 'Wert'])
csv_save_path = '/content/drive/My Drive/CNNMODEL/Hyperparametervgg2.csv'
hyperparameters_df.to_csv(csv_save_path, index=False)
print(f"Hyperparameter gespeichert unter: {csv_save_path}")
...
# Speichern der Trainingsmetriken als CSV in Google Drive
results_df.to_csv('/content/drive/My Drive/CNNMODEL/Training_Metricsmit1Inputvgg2.csv', index=False)
...
# Speichere die Testmetriken als CSV in Google Drive
test_results_df.to_csv('/content/drive/My Drive/CNNMODEL/Test_Metrics_mit_1_Inputvgg2.csv', index=False)

# Speichern des Modells in Google Drive
model_save_path = '/content/drive/My Drive/CNNMODEL/Mein_Trainiertes_Modellmi1Input-vgg2'
model.save(model_save_path)
print(f"Modell gespeichert unter: {model_save_path}")
...
def objective(params):
    ...
    for i in range(params['num_layers']):
        x = BatchNormalization()(x)
        x = Dense(params['units_' + str(i)], activation=params['activation'], kernel_regularizer=l1_l2(l1=params['l1'], l2=params['l2']))(x)
        if params['dropout']:
...
hyperparameters_df = pd.DataFrame(list(best_params.items()), columns=['Hyperparameter', 'Wert'])
csv_save_path = '/content/drive/My Drive/CNNMODEL/Hyperparametervgg12new.csv'
hyperparameters_df.to_csv(csv_save_path, index=False)
print(f"Hyperparameter gespeichert unter: {csv_save_path}")
...
results_df = pd.DataFrame(results)
results_df.to_csv('/content/drive/My Drive/CNNMODEL/Training_Metricsmit1Inputvgg12new.csv', index=False)
...
test_results_df = pd.DataFrame(test_results)
test_results_df.to_csv('/content/drive/My Drive/CNNMODEL/Test_Metrics_mit_1_Inputvgg12new.csv', index=False)

# Save the model
model_save_path = '/content/drive/My Drive/CNNMODEL/Mein_Trainiertes_Modellmi1Input-vgg12'
model.save(model_save_path)
print(f"Modell gespeichert unter: {model_save_path}")
...
plt.ylabel('Mean Squared Error')
plt.legend()
plt.savefig('/content/drive/My Drive/CNNMODEL/MSE_Plotvgg12.pdf')
plt.show()
...
plt.ylabel('Mean Absolute Error')
plt.legend()
plt.savefig('/content/drive/My Drive/CNNMODEL/MAE_Plotvgg12.pdf')
plt.show()