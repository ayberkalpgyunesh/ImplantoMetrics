# -*- coding: utf-8 -*-
"""Xception 130 EndSHAPWerte berechnung.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1kUNJJcN9Ph2JxHNGTylrxqyjJ4xDRoea
"""
...
drive.mount('/content/drive', force_remount=True)
model_path = '/content/drive/My Drive/CNNMODEL/Mein_Trainiertes_Modellmit1Input-Xception5'
model = load_model(model_path)
...
def load_and_extract_features(path_to_data, csv_file, images_folder, selected_features=None, batch_size=18):
    data = pd.read_csv(path_to_data + csv_file, sep=';')
    data['Image Name '] = data['Image Name '] .apply(lambda x: os.path.join(path_to_data + images_folder, f'{x}.tif'))

    datagen = ImageDataGenerator(rescale=1./255)
    generator = datagen.flow_from_dataframe(
        dataframe=data,
        directory='',  # Directory remains empty as paths are now complete
        x_col="Image Name ",
        y_col=["Time", "Cell Radius", "Area", "Migration Radius", "Distrubition of Migration", "Number of Projections", "Compactness"],
        target_size=(299, 299),
        batch_size=batch_size,
    ...
    for _ in range(len(generator)):
        try:
            x, y = generator.next()
            extracted_features = extractor.predict(x)
            if selected_features is not None:
                extracted_features = extracted_features[:, selected_features]
            features.append(extracted_features)
            labels.append(y)
...
# Path to data and datasets
path_to_data = '/content/drive/My Drive/DATASETS3/'
selected_features_indices = list(range(0, 255))
...
if len(train_features_1) == 0 or len(train_labels_1) == 0:
    raise ValueError("Train dataset 1 is empty.")
if len(train_features_2) == 0 or len(train_labels_2) == 0:
    raise ValueError("Train dataset 2 is empty.")
if len(val_features) == 0 or len(val_labels) == 0:
...
# Check if the validation dataset still has valid samples
if len(val_labels) == 0:
    raise ValueError("Validation labels contain no valid samples after processing.")

val_labels_normalized = scaler_time.transform(val_labels[:, 0].reshape(-1, 1)).flatten()

# Prepare training and validation data
X_train_params = combined_labels[:, 1:]  # Measured parameters
X_train_cnn = combined_features  # CNN features
y_train = combined_labels_normalized

X_val_params = val_labels[:, 1:]  # Measured parameters for validation
X_val_cnn = val_features  # CNN features for validation
y_val = val_labels_normalized
...
search_space = {
    ...
    'subsample': uniform(0.5, 0.9 - 0.5),
    'colsample_bytree': uniform(0.5, 0.9 - 0.5)
}

# RandomizedSearchCV for Model A (measured parameters)
...
random_search_A.fit(X_train_params, y_train)

# RandomizedSearchCV for Model B (CNN features)
...
random_search_B.fit(X_train_cnn, y_train)

# Best hyperparameters
print("Best hyperparameters for Model A: ", random_search_A.best_params_)
print("Best hyperparameters for Model B: ", random_search_B.best_params_)

# Model A (measured parameters) evaluation
model_A = random_search_A.best_estimator_
y_pred_val_A = model_A.predict(X_val_params)
mse_A = mean_squared_error(y_val, y_pred_val_A)
mae_A = mean_absolute_error(y_val, y_pred_val_A)
rmse_A = sqrt(mse_A)
...
# Model B (CNN features) evaluation
model_B = random_search_B.best_estimator_
y_pred_val_B = model_B.predict(X_val_cnn)
mse_B = mean_squared_error(y_val, y_pred_val_B)
mae_B = mean_absolute_error(y_val, y_pred_val_B)
rmse_B = sqrt(mse_B)
...
# Erstellen eines Explainers für Modell A
explainer_A = shap.TreeExplainer(model_A)

# Berechnen der SHAP-Werte für Modell A
shap_values_A = explainer_A.shap_values(X_train_params)
...
# CSV-Datei speichern
csv_path = '/content/drive/My Drive/CNNMODEL/ModelA-02.05.2024SHAP_Values_Mean-1.csv'
...
# Mount Google Drive and load the model
drive.mount('/content/drive', force_remount=True)
model_path = '/content/drive/My Drive/CNNMODEL/Mein_Trainiertes_Modellmi1Input-130'
model = load_model(model_path)
...
# Path to data and datasets
path_to_data = '/content/drive/My Drive/DATASETS3/'
selected_features_indices = list(range(0, 127))
...
# Initialize TreeExplainer for Model B
explainer_B = shap.TreeExplainer(model_B)

# Calculate SHAP values for Model B
shap_values_B = explainer_B.shap_values(X_val_cnn)

# Erstellen von Merkmalsnamen für CNN-Features (manuelle Methode)
feature_names_cnn = [f"Feature_{i}" for i in range(128)]

# Summarize the effects of all the features
shap.summary_plot(shap_values_B, X_val_cnn, feature_names=feature_names_cnn)
...
# CSV-Datei speichern
aggregated_shap_values_df.to_csv('/content/drive/My Drive/CNNMODEL/SHAP_ValuesInteraktion02.05.2024.csv', index=False)
...
def load_and_extract_features(path_to_data, csv_file, images_folder, batch_size=18, selected_features=None):
    try:
        ...
        raise ValueError(f"Error loading data from {csv_file}: {e}")

    data['Image Name '] = data['Image Name '].apply(lambda x: f'{x}.tif')
    print(f"Data loaded from {csv_file}: {data.shape[0]} rows")
    ...
    try:
        ...
        raise ValueError(f"Error creating image generator: {e}")
    ...
    for i in range(len(generator)):
        try:
            ...
            extracted_features = extractor.predict(x)  # Ensure extractor is defined elsewhere.
            if selected_features is not None:
                extracted_features = extracted_features[:, selected_features]
            features.append(extracted_features)
            labels.append(y)
    ...
    if original_images and features and labels:
        return np.concatenate(original_images, axis=0), np.concatenate(features, axis=0), np.concatenate(labels, axis=0)
    else:
        raise ValueError("No valid data processed from the generator.")

# Assumed indices of your specific features
selected_features_indices = list(range(0, 128))

# Path to data and datasets
path_to_data = '/content/drive/My Drive/DATASETS3/'

# Loading the original images and labels from the datasets
try:
    original_images_1, extracted_features_1, labels_1 = load_and_extract_features(path_to_data, 'normalized_trainimages1.csv', 'train_images1tif', batch_size=18, selected_features=selected_features_indices)
    original_images_2, extracted_features_2, labels_2 = load_and_extract_features(path_to_data, 'normalized_trainimages2.csv', 'train_images2tif', batch_size=18, selected_features=selected_features_indices)
    original_images_3, extracted_features_3, labels_3 = load_and_extract_features(path_to_data, 'normalized_testimages.csv', 'testimagestif', batch_size=18, selected_features=selected_features_indices)
except ValueError as e:
    raise RuntimeError(f"Error loading data sets: {e}")
...
# Use only a subset of the extracted features
# Use only a subset of the extracted features
subset_extracted_features = np.concatenate([extracted_features_1, extracted_features_2], axis=0)[:100]

# Checking the shape of the input data and the model
print(f"Shape of subset_extracted_features: {subset_extracted_features.shape}")

try:
    shap_values_B = explainer_B.shap_values(subset_extracted_features)
except XGBoostError as xe:
    raise RuntimeError(f"XGBoostError calculating SHAP values: {xe}")
except Exception as e:
    raise RuntimeError(f"Error calculating SHAP values: {e}")

# Aligning the labels to the same subset
# Verwenden Sie die gesamten Labels, ohne sie zu begrenzen
subset_labels = np.concatenate([labels_1, labels_2], axis=0)


# Aggregate SHAP values over time intervals
aggregated_shap_values_list_B = []

# Define max time and time interval
max_time = 144  # e.g., 144 hours total
time_interval = 8  # e.g., 8-hour intervals

# Calculate the aggregated SHAP values for each time interval
for start_time in range(0, max_time, time_interval):
    end_time = start_time + time_interval
...
# Output the aggregated SHAP values for each time interval
for interval in range(len(aggregated_shap_values_array_B)):
    print(f"Time interval {interval * time_interval} to {(interval + 1) * time_interval} hours:")
...
# Aggregate the average SHAP values over time intervals
...
for start_time in range(0, max_time, time_interval):
    ...
    if mask.any():
        ...
        average_shap_values_over_time.append(np.zeros(shap_values_B.shape[1]))


# Convert to a DataFrame for saving as a CSV
...
csv_path = '/content/drive/My Drive/CNNMODEL/MODELBSHAP_Values_Average_Over_Time-1XCEPTION130.csv'
